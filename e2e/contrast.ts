 ...
 *
 *  - text over a surface axe declines to resolve. Every dramatic surface in
 *    this lab is one: `.hex-output` is `rgba(0,0,0,0.4)` over the panel,
 *    `.attack-overlay` is `rgba(255,45,45,0.05)`, the "why this matters"
 *    notice is `rgba(255,170,0,0.05)`, the attack theater's "YOUR MOVE"
 *    callout is `rgba(255,0,0,0.04)`, `--green-glow` / `--red-glow` are
 *    translucent again, and the hero aside is a `color-mix(in oklab, …)`.
 *    axe files those nodes under `incomplete` rather than measuring them, so a
 *    gate asserting only on `violations` never sees the ink the whole exhibit
 *    is painted in.
 ...
 *    at `.85`, and `body`'s own `crt-flicker` keyframes, which dip the entire
 *    document to `opacity: 0.97` on a 4s loop for anyone who has not asked for
 *    reduced motion.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Three boxes here
 *    clip live text: `.hex-output`, capped at 8rem (6rem on a phone) around
 *    generator output; `.modal-content`, capped at 80vh around the KAT and
 *    ABOUT dialogs; and `.table-scroll`, the wrapper each statistics table
 *    sits in, whose cells are all `white-space: nowrap`. Content scrolled past any of them is not dimmed or partly drawn:
 ...
 *    walk would find nothing behind it, fall through to white, and invent a
 ...
 * uses. Nothing here is short-lived enough to need narrowing: the one state
 * that ends on its own schedule — "Generating the lab PKI…", which lasts about
 * 70ms — is instead held open at the network layer and scanned in full, so the
 * gate has no state it can only sample. The parameter is kept because that is
 * a property of this lab today rather than of the helper, and the narrowed form
 * is the only honest way to measure a state shorter than an axe pass: a
 * whole-page scan would catch it sometimes, and a sometimes-measured assertion
 * is worse than none.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: the hero aside is
     * `color-mix(in oklab, var(--green-clean) 6%, transparent)`, and Chromium
     * reports that to `getComputedStyle` unchanged rather than converting it
     * to sRGB. A regex that only understands `rgb()/rgba()` sees `null` for it
     * and the walk then falls through to the wrong backdrop — which
     ...
     *
     * THE ONE GRADIENT ON THIS PAGE IS THE CRT SCANLINE OVERLAY: `body::after`
     * paints `repeating-linear-gradient(0deg, transparent, transparent 2px,
     * rgba(0,0,0,0.03) 2px, rgba(0,0,0,0.03) 4px)` fixed over the whole
     * document at `z-index: 9999`. It is not composited by this walk and must
     * not be, for two reasons: it is a SIBLING pseudo-element rather than an
     * ancestor, and this gate runs with `prefers-reduced-motion: reduce`, under
     * which the lab sets `body::after { display: none }` and the overlay is not
     * painted at all. What it costs a reader who has NOT asked for reduced
     * motion is bounded and small — a 50% duty cycle of 3% black is 1.5% black
     * on average, which moves a 4.5:1 pair by well under 0.05 — but it is a
     * real difference between what this gate measures and what that reader
     * sees, and it is stated here rather than left implicit.
     *
     * The sampler is kept for the ordinary gradient case: `paintAt` has to
     * decide what a `background-image` layer contributes, and the alternative
     * default — treat every layer as transparent — is silently wrong the moment
     * anyone gives a panel or the progress bar a gradient. The walk would then
     * composite the text onto the surface BEHIND the gradient and report a
     * ratio nothing on screen has, in the direction that hides failures.
     *
     * Sampling, rather than judging a gradient at its worst stop: a stop-worst
     * reading assumes the worst stop covers the text wherever the text sits,
     * which is false for any gradient taller or wider than one line. So each
     * gradient is evaluated at the text's real location — linear by projecting
     ...
     *
     * A driven scan here walks three bit heatmaps of 240 `.bit-cell` divs
     * each, a four-row statistics table, an eleven-row prediction table, the
     * attack theater's explainer, and a KAT modal of five vector rows — and the
     * heatmap cells alone are 720 elements that each re-walk the same handful
     * of ancestors up to `<body>`. (They own no text, so the walk skips them
     * early, but the rect and style lookups on their ancestors are shared with
     * everything that does.) Without the caches the pass re-reads the same
     * computed styles and rects thousands of times, and the gate scans a dozen
     * states per configuration across four configurations. Nothing mutates the
     ...
     * that is `.hex-output`, `.modal-content`, `.progress-bar`, `.table-scroll`
     * around each statistics table, and `#sr-announcer`, whose 1x1
     * `overflow: hidden` box is how status messages reach a screen reader
     * without being drawn.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This lab draws no SVG of its own at all — the only
      // SVG on the page is the shared header's hamburger and GitHub marks — so
      // the filter is inert here; it is the guard that keeps the first figure
      // anyone adds honest.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab has no
      // `<details>` at all; what it has instead is a dozen regions hidden by
      // inline `style.display = 'none'` — the attack container, the prediction
      // badge, the intercepted-output block, the recovered-state line, the
      // predictions header and the "YOUR MOVE" callout. The gate this replaces
      // walked EVERY element on the page and cleared any inline
      // `display: none` it found, producing a document no visitor can load:
      // the attack theater fully expanded with its progress bar at 0%, its
      // prediction table empty and its summary blank, all at once. Every one of
      // those regions is reached here by driving the attack that reveals it.
     ...
     * catch what axe *misses* among exposed text (tints, opacity), not to be
     * stricter than axe on decorative content. Honour the same boundary.
     *
     * Note the boundary cuts both ways and is a known gap: text that is
     * `aria-hidden` but still painted is skipped by BOTH oracles, so nothing
     * here may hide a real value behind that attribute. That was audited on
     * this page. Two things carry it and neither holds a value: the shared
     * header's hamburger and GitHub glyphs, and each panel's `.status-icon`
     * emoji, whose meaning is carried by the panel's own heading text beside
     * it. The `.hex-output` blocks, the bit heatmaps, the statistics table and
     * every row of the prediction table are exposed, and are measured.
     */
     ...
     *
     * This lab draws no SVG of its own, so the guard is inert today. It stays
     * because the mistake is cheap to make and silent, and this page builds
     * almost everything through `innerHTML` template literals, which is exactly
     * where it happens: interpolating a string ARRAY into a template literal
     * (`<g>${rows}</g>`) makes JS join it with commas, leaving a run of ","
     * text nodes inside the group. They render nothing, but a walk that
     * measures them reports a 1:1 failure against the panel — a ratio
     ...
     * box (CSS Backgrounds 3, "The Canvas Background"); if the root's
     * background is transparent the value is taken from <body> instead.
     *
     * This lab makes that distinction load-bearing, which is how it was found.
     * It sets `html, body { height: 100% }`, so BOTH boxes are exactly one
     * viewport tall while the document runs several viewports long. Every
     * element below the fold — the whole "why this matters" notice, the entire
     * footer — intersects neither box, so the walk ended with a transparent
     * backdrop and fell through to WHITE. In the DARK theme that reported
     * `rgb(136,136,160)` on `rgb(255,255,255)` at 3.46:1 for footer text whose
     * real ratio against `--bg-primary` (#0a0a0f) is 5.63:1: nine invented
     * failures on the first scan, describing a white page that does not exist.
     *
     * So the canvas is composited under whatever the ancestor walk accumulated,
     * before the final fallback to white. Text inside an opaque panel never
     * reaches this — the walk breaks out as soon as the backdrop is opaque.
     */
